...
그래서 데이터 엔지니어가 시스템 디자인을 공부해야하는 이유를 고민하고 정리한 후, 지인이 추천해준 *Grokking System Design Interview* 라는 자료를 보며 시스템 디자인을 공부하였다.
...
예를 들면 학습된 ML 아티팩트를 서버로 어떻게 배포할지, CPU와 GPU 리소스를 어떻게 분배할지, SLA를 맞추기 위해 비동기와 캐싱을 활용하는 방식 등을 시스템 설계로부터 도움받을 수 있었다.
...
코딩 인터뷰와 마찬가지로, SDI에 충분히 노력을 기울이지 못한다면 Google이나 Amazon과 같은 최고 기업에서 성과를 내기 어렵다고 여겨진다. 시스템 설계에 능숙한 사람들은 복잡한 시스템을 다룰 수 있는 능력을 면접에서 보여줄 수 있기 때문에 더 좋은 오퍼를 받게된다.

따라서 이 글에서는 디자인 문제를 풀 수 있도록 다음과 같이 step-by-step으로 따라가도록 한다.

### Step 1: 요구 사항을 명확히 하기

디자인 관련 질문들은 단 하나의 정답만 있는 것이 아니기 때문에 풀고자 하는 문제의 범위를 명확히 하는 것이 중요하다. 설계는 정답이 정해져 있지 않고, 단 하나의 정답만 존재하는 것도 아니기 때문에 인터뷰 초기에 엄밀함을 명확히 하는 것이 중요하다. 시스템의 목표를 정의하는데 충분한 시간을 들인 후보자는 인터뷰에서 더 성공적인 기회를 얻게 된다. 또한 우리는 35~40분 간 큰 시스템을 디자인해야 하기 때문에, 우리가 집중해야할 시스템이 무엇인지를 아는 것이 중요하다.

만약 Twitter와 같은 서비스를 만든다고 치자. 그러면 Twitter를 설계하기 위해 이러한 질문들을 던져 볼 수 있을 것이다.

- 서비스를 사용하는 유저들은 tweet을 게시하고 다른 사람들을 팔로우할 수 있도록 할 수 있을까?
- 유저의 타임라인을 생성하고 보여줄 수 있을까?
- Tweet에 사진과 동영상을 담을 수 있을까?
- 백엔드에만 집중할 것인가 아니면 프론트 엔드 또한 개발할 것인가
- 유저들은 tweet을 검색할 수 있는가
- 지금 유행하는 핫한 주제를 보여줄 수 있을까?
- 새로운 tweet에 대한 알람을 보낼 수 있을까?

### Step 2: 시스템 인터페이스 정의하기

시스템이 요구하는 API를 정의하고 요구사항에 잘못된 것은 없는지 점검해야 한다.
...
디자인하고자 하는 시스템의 규모를 예상하고 scaling, partitioning, load balancing, caching 전략 사용을 검토
...
시스템에 필요한 데이터의 엔티티를 정리하며 저장, 전송, 암호화와 같은 데이터 관리 측면 또한 고려

또한 사용할 데이터베이스의 성격( SQL v.s. NoSQL) 고려
