...
매튜 상관계수는 phi coefficient라고도 불리며 옳고 그름을 판별하는 이진분류(binary classification)에 사용되는 metric이다. 여느 다른 상관계수처럼 -1과 1 사이의 값을 가지고, 1에 가까울수록 측정하고자 하는 두 관측치가 "비슷하다"고 본다.

위키피디아를 보면 매튜 상관계수에 접근하는 다양한 계산 방식을 살펴볼 수 있다. 먼저 confusion matrix를 계산하기 위해 TP, FP, FN, TN을 이용하는데, 이 지표들은 Recall, Precision, F1-score 등과 관련이 깊다.
...
여기서 중요한 점은 매튜 상관계수가 사실 두 벡터 $X$, $Y$의 피어슨 상관계수라는 것이다. $X$와 $Y$의 피어슨 상관계수 $\rho$를 구해보면 MCC와 같은 값을 갖는다.
...
다만, 이렇게 피어슨 상관계수를 통해 계산하는 방법은 비효율적일 수도 있다. 추천시스템의 경우 user와 item에 대한 interactive matrix를 만들어 row-wise하게 계산해야 하는데, 이는 메모리 관리에 있어서 상당히 부담스러울 수 있다.
