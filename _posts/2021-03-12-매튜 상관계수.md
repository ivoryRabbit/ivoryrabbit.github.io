...
상관계수(correlation coefficient)란 두 변수가 ordinal scale로 주어졌을 때 둘 사이에 얼마나 상관관계가 있는지 측정하는 척도이다. 보통 양의 상관관계를 가질 때 상관계수가 크고 음의 상관관계를 가질 때 상관계수가 작도록 설계되어 있다. 추천시스템에서는 두 아이템 혹은 두 고객이 얼마나 유사한지 계산하는 거리개념으로 활용되기도 한다.
