...
테이블의 변경 사항을 history 형태로 남기고 싶을 때 사용한다. 테이블이 SCD(Slowly Changing Dimension)인 경우에 유용하며, 데이터의 추가나 수정이 언제 발생했는지 알 수 있도록 `dbt_valid_from`과 `dbt_valid_to` 라는 두 컬럼이 붙은 새로운 테이블을 생성해준다.
