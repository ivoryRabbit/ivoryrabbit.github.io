...
Data Ingestion 방식에는 크게 두 가지 유형이 있다. 각 방식에 따라 파이프라인 개발 시 어떠한 점들을 고려해야 할지 정리해보았다.
...
지금까지의 데이터 엔지니어링 경험을 보태자면, 많은 회사들이 다양한 패턴의 Data Ingestion 파이프라인들을 운영하고 있다. 그 중 가장 흔하게 보이는 패턴들은 다음과 같았다.
