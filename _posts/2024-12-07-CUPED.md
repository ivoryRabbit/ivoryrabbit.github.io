...
위의 t-test 수식을 놓고 곰곰이 생각해보면 추정량의 표준편차, 즉 표준오차(Standard Error, SE)가 작아질 수록 p-value가 작아진다.
...
결국 A/B 테스트에서 유의수준 보다 낮은 p-value를 얻기 위해서는 결국 표준오차가 작아질 때까지 표본 수가 쌓이길 기다려야 한다.

실제로도 예시에서 표본 수를 1000개로 늘린다면 p-value가 떨어지는 것을 볼 수 있다.
...
온라인 실험에서는 민감도가 중요하다. 왜 중요하냐?

1. 미미한 효과를 감지하기 위해
민감도는 사용자 수 제곱에 반비례하기 때문에 5%의 변화를 감지하기 위해 1만명이 필요하다면, 0.5%의 변화를 감지하기 위해서는 100만명이 필요하다.
2. 빠른 결과 도출
긍정적인 기능은 빨리 출시하고, 부정적인 기능은 빠르게 중단해야한다.
3. 작은 표본
특정 세그먼트에만 영향을 주는 기능이라면 표본 수가 작을 수 있다.
4. 실험 수요 증가
실험 수가 많아지면 트래픽 최적화가 필요하다.

그러면 민감도를 어떻게 높이는가?
-> 분산을 감소시키면된다

지금까지 분산을 감소시키는 연구들은 특정 상황에서만 사용가능
-> 범용적인 방법이 필요하다

논문은 CUPED(Controlled-experiment Using Pre-Experiment Data) 기법을 제안

-> 대조군과 실험군에 대해사전 실험 데이터(Pre-Experiment Data)를 활용하여 실험 결과의 분산을 줄이고 민감도를 높이는 방법

- CUPED는 사전 데이터를 사용해 온라인 실험의 분산을 줄이고 민감도를 크게 향상
- 비사용자 기반 지표와 일부 사전 데이터가 누락된 경우에도 적용 가능
- 사전 실험 단계에서 동일 지표를 사용하는 것이 가장 큰 분산 감소를 가져온다는 경험적 결과를 포함
- Bing에서 실행된 실제 온라인 실험에서 약 50%의 분산 감소를 검증했으며, 이는 트래픽을 두 배로 늘리거나 실험 시간을 절반으로 줄이는 것과 같은 효과
- 사전 실험 기간의 최적 길이와 다중 공변량 사용 등 성공적인 CUPED 적용을 위한 중요한 선택 요소를 제공
