...
pgvector는 초기에 IVF-Flat 만 지원하다 최근 HNSW가 추가되었다. Contributor 피셜로는 IVF-PQ와 ScaNN 알고리즘이 추가될 예정이라 한다.

다음 쿼리를 이용하면 IVF Index를 추가할 수 있다. 여기서 lists와 probes는 각각 "생성할 클러스터의 개수"와 "탐색할 클러스터의 개수"를 의미한다.
...
참고로 테이블 하나에 여러 Index를 추가할 수도 있고, segment 별로 서로 다른 Index를 운영할 수도 있다.
